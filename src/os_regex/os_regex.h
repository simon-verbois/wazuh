...
/**
 * @brief Compare an already compiled OSRegex regular expression with a not NULL string
 * 
 * @warning Do not mix between `OSRegex_Execute` and `OSRegex_Execute_ex` calls for the same reg
 ...
 * @brief Compare an already compiled OSRegex regular expression with a not NULL string
 * 
 * Extension of OSRegex_Execute that allows to choose external sub_strings and prts_str.
 ...
 * all calls to with the same `reg` must have a null regex_match argument (This limits the parallelism)
 * @warning Do not mix between `OSRegex_Execute_ex` and `OSRegex_Execute` calls for the same reg
 * 
 * @param str string to test
 * @param reg compiled pattern
